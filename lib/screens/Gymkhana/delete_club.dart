...
//           if (clubs.isEmpty) {
//             return Center(child: Text('No clubs to delete'));
//           }
      ...
      body: FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            List<dynamic> clubs = snapshot.data!;
            // Filter clubs with status 'confirmed'
            List<dynamic> confirmedClubs = clubs.where((club) => club['status'] == 'confirmed').toList();

            if (confirmedClubs.isEmpty) {
              return Center(child: Text('No clubs to delete'));
            }
            return SingleChildScrollView(
              scrollDirection: Axis.vertical,
              child: SingleChildScrollView(
                ...
                child: DataTable(
                  ...
                  rows: confirmedClubs.map<DataRow>(
                    (club) => DataRow(
                      cells: <DataCell>[
                        DataCell(
                          Text(
